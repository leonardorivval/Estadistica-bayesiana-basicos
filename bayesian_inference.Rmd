...
```{r Q1-summay}
# A way of seeing this is by the function 
glimpse(nc)
```
...
Answer: is a little bit left skewed
</div>
...
Without the non_full term kids th data appears to be normal
</div>
...
Answer: * There is a 95% chance that babies on average weigh 7.4 to 7.5 pounds.
</div>
...
The answer is: BF[H2:H1] = 25478497, so it is very, very strong. So strong that P(H2|data) = 1!! 
</div>

```{r Q4-HT-weight}
# Type your code for the Exercise here.

bayes_inference(y=weight, data = nc_fullterm, type = "ht", statistic = "mean",
                alternative = "twosided", prior_family = "JZS",
                mu_0 = 7.7, rscale = 1, method = "theoretical", show_plot = FALSE)
```
...
The answer is: there is a 95% chance that their birth weight will be 5.4 to 9.5 pounds.
...
```{r E2-t-dist}
#First, we generate the data for premies only
nc_premie= filter(nc, premie=="premie")
summary(nc_premie$premie)

#Then we can plot the data
ggplot(data = nc_premie, aes(x=weight))+
  geom_histogram(fill="light blue", color="black")

#Now we apply the bayesian inference
weight_post_1=bayes_inference(y=weight, data = nc_premie, type = "ht",
                method = "sim", statistic = "mean", prior_family = "JZS",
                alternative = "twosided",mu_0 = 0, rscale = 1)

#Generate a predictive distribution
names(weight_post_1)
sample_1=as.data.frame(weight_post_1$samples)
nsim_1 = nrow(sample_1)
sample_1 = mutate(sample_1, y_pred = rnorm(nsim_1, mu, sqrt(sig2)))

ggplot(data = sample_1, aes(x=y_pred))+
  geom_histogram(aes(y=..density..), bins = 100)+
  geom_density()+
  xlab(expression(y[new]))

#Credible intervals of predictive pistribution
dplyr::select(sample_1, mu, y_pred)%>%
  map(quantile, probs=c(.025, .50, .975))
...
The answer is: The IQRs of the distributions are roughly equal.
</div>
...
The answer: the bayes factor is 6.237, therefore it is a positive BF since it is between 3-20
</div>
...
# Type your code for the question here.
bayes_inference(y=weight, x=habit, data = nc_fullterm, type = "ht",
                prior_family = "JZS", hypothesis_prior = c(.25 , 0.75),
                method = "theoretical", null=0, alternative= "twosided",
                statistic = "mean", show_plot = FALSE)
```
...
bayes_inference(y=weight, x=habit, data=nc_fullterm, type="ht",
                prior_family = "JZS", alternative = "twosided", rscale = sqrt(2)/2,
                null = 0, show_plot = F,method = "theoretical", statistic = "mean" )
```
...
The answer:  there is a 95% chance that babies born to nonsmoker mothers are on average 0.11 to 0.54 pounds heavier at birth than babies born to smoker mothers.
</div>

```{r Q10-ci-weight-habit-increase}
# Type your code for Question 10 here.
bayes_inference(y=weight, x=habit, data=nc_fullterm, type = "ci",
                prior_family = "JZS", statistic = "mean", mu_0=0, 
                method = "theoretical")
```
...
bayes_inference(y=diff, data= pss_sport, type = "ht", prior = "JZS",
                null = 0, rscale = 1, method = "simulation", 
                alternative = "twosided", statistic = "mean", 
                show_plot = FALSE)
...
dff_both = rbind(pss_sport, pss_control)

bayes_inference(x=Group, y=diff, data=dff_both, type = "ht", null = 0, 
                prior_family = "JZS", rscale = 1, method = "simulation",
                statistic = "mean", alternative = "twosided", show_plot = F)
