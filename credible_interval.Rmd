...
qbeta(c(.05, .95), shape1=2, shape2 = 5)
```
...
qgamma(c(.005, .995), shape=4, rate=8)
```
...
The answer is:
* The probability that the true proportion of females lies in this interval is 0.95. Since bayesian credible intervals anable us to talk about the probability of a parameter being in a certain interval.
</div>
<br>
...
The answer is:
qbeta(c(0.025, 0.975), shape1 = 3086, shape2 = 2915)
## [1] 0.502 0.527
</div>
...
a<-qbeta(c(0.025, 0.975), shape1 = 5, shape2 = 200)
a
(0.049440288 + 0.008005099)/2
```
...
qbeta(c(0.05, 0.95), shape1 = 3869, shape2 = 1132)
## [1] 0.764 0.783
</div>
...
THe correct answer is Gamma(22,6)
</div>
...
a_1<-5
a_2<-1
c <- brfss$fruit_per_day
a_1post <- a_1 + sum(c)
a_2post <- a_2 + length(c)
a_1post
a_2post
```
...
No, they eat between 1.594 and 1.653 pieces of fruit a day!
</div>
...
We a a posterior gamma(5434, 5001), and a credible interval between  1.062451 and 1.110941
</div>
...
gamma1 <- 5
gamma2 <- 1
g <- brfss$vege_per_day
gamma1post <- gamma1 + sum(g)
gamma2post <- gamma2 + length(g)
gamma1post
gamma2post

qgamma(c(c(0.05, 0.95)), shape = gamma1post, rate=gamma2post)
```
